...
class GestionController extends AbstractController
{
    ...
    public function mesEnfants(
        ...
        $ecoles = $ecoleRepository->findValides();
        ...
        if ($request->isMethod('POST') && $request->request->get('action') === 'ajouter_enfant') {
            ...
            if ($nom && $prenom && $dateNaissance && $sexe && $ecoleId && $certificatFile) {
                // Vérifier que l'école existe et est validée
                $ecole = $ecoleRepository->find($ecoleId);
                if (!$ecole || !$ecole->isValide()) {
    ...
    #[IsGranted('ROLE_USER')]
    public function modifierEnfant(Request $request, int $id, EnfantRepository $enfantRepository, EcoleRepository $ecoleRepository, EntityManagerInterface $entityManager): Response
    {
        $error = null;
        $success = null;
        $enfant = null;
