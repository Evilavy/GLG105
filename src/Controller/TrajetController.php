            ...
            if ($departDateTime <= $now) {
                ...
                try {
                    ...
                    if ($response->getStatusCode() === 201) {
                        ...
                        // Ajouter des points au conducteur via PointsApi
                        try {
                            $pointsApiUrl = 'http://localhost:5164/api/points';
                            $pointsAjout = 10; // ou la valeur souhaitée
                            $pointsResponse = $this->httpClient->request('POST', $pointsApiUrl . '/add', [
                                'json' => [
                                    'id' => $userId,
                                    'points' => $pointsAjout
                                ]
                            ...
                            if ($pointsResponse->getStatusCode() === 200) {
                                $success .= ' Vous avez gagné ' . $pointsAjout . ' points !';
                            } else {
                                $success .= ' (Erreur lors de l\'ajout des points)';
                            }
                        } catch (\Exception $e) {
                            $success .= ' (Erreur lors de l\'ajout des points : ' . $e->getMessage() . ')';
                        }
                    } else {
    ...
    public function supprimerTrajet(Request $request, int $id, EnfantRepository $enfantRepository): Response
    {
        ...
        try {
            ...
            if ($trajetResponse->getStatusCode() === 200) {
                ...
                // Déduire les points gagnés par le conducteur lors de la création du trajet
                // Le conducteur perd exactement les 10 points qu'il a gagnés lors de la création
                $pointsGagnes = 10; // Même montant que lors de la création
                try {
                    $pointsResponse = $this->httpClient->request('POST', 'http://localhost:5164/api/points/remove', [
                        'json' => ['id' => $userId, 'points' => $pointsGagnes]
                    ]);
                    if ($pointsResponse->getStatusCode() !== 200) {
                        error_log("Erreur déduction points conducteur - User ID: $userId, Points: $pointsGagnes");
                    }
                } catch (\Exception $e) {
                    // Log l'erreur mais continuer la suppression
                    error_log("Exception déduction points conducteur - User ID: $userId, Error: " . $e->getMessage());
                }
                ...
                if ($deleteResponse->getStatusCode() === 204) {
                    if (!empty($enfantsIds)) {
                        ...
                        $this->addFlash('warning', "Vous avez perdu 10 points gagnés lors de la création de ce trajet.");
